...
class TAG(EncoderLayer):
    ...
    def __init__(self, device='cpu', pocket_num=30, output_feats=128, pooling=False):
        """
    ...
    def forward(self, g):
        ...
        if self.pooling:
            ...
            for num_nodes in num_nodes_per_graph:
                pos_embed = g.ndata['pocket_number'][start_idx]
                positional_embeddings.append(pos_embed)
                start_idx += num_nodes

            positional_embeddings = torch.stack(positional_embeddings).view(B, -1)
            return protein_rep_reshaped, positional_embeddings  
